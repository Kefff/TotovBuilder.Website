import { IPrice } from '../../models/item/IPrice'
import { ak12PistolGrip, ak12Stock, ak12bt, alkali, alpha, ammo1270Magnum, ammo545us, ammo9mmGT, aquamari, armbandBlue, armor6b13Fl, armor6b13FlDefault, banshee, bansheeDefault, bayonet6Kh5, berkut, capacitors, cf, crossbow, cultLocust, ekp802dt, erBayonet, esLamp, eur, ewr, filter, h2o2, halfMask, iskra, k1s, k1sVisor, lshZ2dtm, lshZ2dtmFs, m9a3, m9a3Cap, m9a3Default, m9a3Fs, m9a3Magazine, m9a3Rs, m9a3Slide, m9a3Thr, mechanism, monocletePe, morphine, ms2000, mts25512, mts25512755mmRib, mts25512Default, mts25512choke, mts25512cyl, nf30mm, opSksDefault, opSksDt, paca, paid, pass, plate6b33Back, plate6b33Front, plexiglass, precision, pso1, razor, rgd5, rooster, rpk16, rpk1615inch, rpk16Default, rpk16Drum, rpk16DustCover, rpk16Handguard, rpk16MuzzleBreak, rpk16Rail, rpk16Rs, rpk16RsBase, rpk16Tube, rub, salewa, scavVest, slickers, specterDr, srd9, syringe, usd, vaseline, vhs, vita, wParts, water, x400 } from './itemMocks'

export const ak12btPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ak12bt.id,
    merchant: 'prapor',
    merchantLevel: 3,
    quest: undefined,
    value: 3636,
    valueInMainCurrency: 3636
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ak12bt.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 120000,
    valueInMainCurrency: 120000
  }
]

export const ak12PistolGripPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ak12PistolGrip.id,
    merchant: 'prapor',
    merchantLevel: 2,
    quest: undefined,
    value: 2163,
    valueInMainCurrency: 2163
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ak12PistolGrip.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 12318,
    valueInMainCurrency: 12318
  }
]

export const ak12StockPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ak12Stock.id,
    merchant: 'prapor',
    merchantLevel: 3,
    quest: undefined,
    value: 5641,
    valueInMainCurrency: 5641
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ak12Stock.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 15225,
    valueInMainCurrency: 15225
  }
]

export const alkaliPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: alkali.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 12109,
    valueInMainCurrency: 12109
  }
]

export const alphaPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'USD',
    itemId: alpha.id,
    merchant: 'peacekeeper',
    merchantLevel: 2,
    quest: undefined,
    value: 7158,
    valueInMainCurrency: 1023660
  }
]

export const ammo1270MagnumPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ammo1270Magnum.id,
    merchant: 'jaeger',
    merchantLevel: 3,
    quest: undefined,
    value: 200,
    valueInMainCurrency: 200
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ammo1270Magnum.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 706,
    valueInMainCurrency: 706
  }
]

export const ammo545bpPrices: IPrice[] = []

export const ammo545usPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ammo545us.id,
    merchant: 'prapor',
    merchantLevel: 1,
    quest: undefined,
    value: 96,
    valueInMainCurrency: 96
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ammo545us.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 137,
    valueInMainCurrency: 137
  }
]

export const ammo9mmGTPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ammo9mmGT.id,
    merchant: 'mechanic',
    merchantLevel: 1,
    quest: undefined,
    value: 73,
    valueInMainCurrency: 73
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ammo9mmGT.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 102,
    valueInMainCurrency: 102
  }
]

export const aquamariPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: aquamari.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 28774,
    valueInMainCurrency: 28774
  }
]

export const armbandBluePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: armbandBlue.id,
    merchant: 'ragman',
    merchantLevel: 1,
    quest: undefined,
    value: 637,
    valueInMainCurrency: 637
  }
]

export const armor6b13FlPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: armor6b13Fl.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 66048,
    valueInMainCurrency: 66048
  }
]

export const armor6b13FlDefaultPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: armor6b13FlDefault.id,
    merchant: 'ragman',
    merchantLevel: 2,
    quest: {
      id: '638fcd23dc65553116701d33',
      name: 'Audit',
      wikiLink: 'https://escapefromtarkov.fandom.com/wiki/Audit'
    },
    value: 64269,
    valueInMainCurrency: 64269
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: armor6b13FlDefault.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 74902,
    valueInMainCurrency: 74902
  }
]

export const bansheePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: banshee.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 33950,
    valueInMainCurrency: 33950
  }
]

export const bansheeDefaultPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: bansheeDefault.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 63600,
    valueInMainCurrency: 63600
  },
  {
    barterItems: [
      {
        itemId: ewr.id,
        quantity: 1
      },
      {
        itemId: vhs.id,
        quantity: 1
      }
    ],
    currencyName: 'barter',
    itemId: bansheeDefault.id,
    merchant: 'ragman',
    merchantLevel: 3,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const bayonet6Kh5Prices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: bayonet6Kh5.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 24924,
    valueInMainCurrency: 24924
  },
  {
    barterItems: [
      {
        itemId: filter.id,
        quantity: 1
      },
      {
        itemId: aquamari.id,
        quantity: 2
      }
    ],
    currencyName: 'barter',
    itemId: bayonet6Kh5.id,
    merchant: 'prapor',
    merchantLevel: 2,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const berkutPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: berkut.id,
    merchant: 'ragman',
    merchantLevel: 2,
    quest: undefined,
    value: 24509,
    valueInMainCurrency: 24509
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: berkut.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 27285,
    valueInMainCurrency: 27285
  }
]

export const capacitorsPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: capacitors.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 15510,
    valueInMainCurrency: 15510
  }
]

export const cfPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: cf.id,
    merchant: 'ragman',
    merchantLevel: 2,
    quest: undefined,
    value: 4793,
    valueInMainCurrency: 4793
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: cf.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 16816,
    valueInMainCurrency: 16816
  }
]

export const crossbowPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: crossbow.id,
    merchant: 'ragman',
    merchantLevel: 2,
    quest: undefined,
    value: 3885,
    valueInMainCurrency: 3885
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: crossbow.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 9460,
    valueInMainCurrency: 9460
  }
]

export const cultLocustPrices: IPrice[] = [
  {
    barterItems: [
      {
        itemId: rooster.id,
        quantity: 1
      }
    ],
    currencyName: 'barter',
    itemId: cultLocust.id,
    merchant: 'skier',
    merchantLevel: 4,
    quest: {
      id: '5c0d4c12d09282029f539173',
      name: 'Peacekeeping Mission',
      wikiLink: 'https://escapefromtarkov.fandom.com/wiki/Peacekeeping_Mission'
    },
    value: 0,
    valueInMainCurrency: 0
  }
]

export const ekp802dtPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ekp802dt.id,
    merchant: 'prapor',
    merchantLevel: 1,
    quest: undefined,
    value: 10194,
    valueInMainCurrency: 10194
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ekp802dt.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 10645,
    valueInMainCurrency: 10645
  }
]

export const erBayonetPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: erBayonet.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 13656,
    valueInMainCurrency: 13656
  }
]

export const esLampPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: esLamp.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 25816,
    valueInMainCurrency: 25816
  }
]

export const eurPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: eur.id,
    merchant: 'skier',
    merchantLevel: 1,
    quest: undefined,
    value: 159,
    valueInMainCurrency: 159
  }
]

export const ewrPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ewr.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 21923,
    valueInMainCurrency: 21923
  }
]

export const filterPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: filter.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 44924,
    valueInMainCurrency: 44924
  }
]

export const h2o2Prices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: h2o2.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 11473,
    valueInMainCurrency: 11473
  }
]

export const halfMaskPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: halfMask.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 11521,
    valueInMainCurrency: 11521
  }
]

export const iskraPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: iskra.id,
    merchant: 'jaeger',
    merchantLevel: 2,
    quest: {
      id: '5d25b6be86f77444001e1b89',
      name: 'The Survivalist Path - Thrifty',
      wikiLink: 'https://escapefromtarkov.fandom.com/wiki/The_Survivalist_Path_-_Thrifty'
    },
    value: 24392,
    valueInMainCurrency: 24392
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: iskra.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 28714,
    valueInMainCurrency: 28714
  },
  {
    barterItems: [
      {
        itemId: syringe.id,
        quantity: 1
      }
    ],
    currencyName: 'barter',
    itemId: iskra.id,
    merchant: 'therapist',
    merchantLevel: 1,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const k1sPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: k1s.id,
    merchant: 'ragman',
    merchantLevel: 1,
    quest: undefined,
    value: 8879,
    valueInMainCurrency: 8879
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: k1s.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 8897,
    valueInMainCurrency: 8897
  }
]

export const k1sVisorPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: k1sVisor.id,
    merchant: 'ragman',
    merchantLevel: 1,
    quest: undefined,
    value: 5385,
    valueInMainCurrency: 5385
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: k1sVisor.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 7048,
    valueInMainCurrency: 7048
  }
]

export const lshZ2dtmPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: lshZ2dtm.id,
    merchant: 'ragman',
    merchantLevel: 4,
    quest: undefined,
    value: 86801,
    valueInMainCurrency: 86801
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: lshZ2dtm.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 63493,
    valueInMainCurrency: 63493
  }
]

export const lshZ2dtmFsPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: lshZ2dtmFs.id,
    merchant: 'ragman',
    merchantLevel: 4,
    quest: undefined,
    value: 37019,
    valueInMainCurrency: 37019
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: lshZ2dtmFs.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 50084,
    valueInMainCurrency: 50084
  },
  {
    barterItems: [
      {
        itemId: plexiglass.id,
        quantity: 3
      }
    ],
    currencyName: 'barter',
    itemId: lshZ2dtmFs.id,
    merchant: 'ragman',
    merchantLevel: 3,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const m9a3Prices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'USD',
    itemId: m9a3.id,
    merchant: 'peacekeeper',
    merchantLevel: 1,
    quest: undefined,
    value: 75,
    valueInMainCurrency: 8025
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: m9a3.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 17511,
    valueInMainCurrency: 17511
  }
]

export const m9a3DefaultPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'USD',
    itemId: m9a3Default.id,
    merchant: 'peacekeeper',
    merchantLevel: 1,
    quest: undefined,
    value: 107,
    valueInMainCurrency: 15337
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: m9a3Default.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 17511,
    valueInMainCurrency: 17511
  }
]

export const m9a3CapPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: m9a3Cap.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 15000,
    valueInMainCurrency: 15000
  }
]

export const m9a3FsPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: m9a3Fs.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 10000,
    valueInMainCurrency: 10000
  }
]

export const m9a3MagazinePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'USD',
    itemId: m9a3Magazine.id,
    merchant: 'peacekeeper',
    merchantLevel: 1,
    quest: undefined,
    value: 15,
    valueInMainCurrency: 2178
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: m9a3Magazine.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 11342,
    valueInMainCurrency: 11342
  }
]

export const m9a3RsPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: m9a3Rs.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 24700,
    valueInMainCurrency: 24700
  }
]

export const m9a3SideGripPrices: IPrice[] = []

export const m9a3SlidePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: m9a3Slide.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 50000,
    valueInMainCurrency: 50000
  }
]

export const m9a3ThrPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: m9a3Thr.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 29709,
    valueInMainCurrency: 29709
  }
]

export const mechanismPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: mechanism.id,
    merchant: 'ragman',
    merchantLevel: 2,
    quest: undefined,
    value: 67446,
    valueInMainCurrency: 67446
  }
]

export const morphinePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: morphine.id,
    merchant: 'therapist',
    merchantLevel: 4,
    quest: {
      id: '5a68665c86f774255929b4c7',
      name: 'Health Care Privacy - Part 3',
      wikiLink: 'https://escapefromtarkov.fandom.com/wiki/Health_Care_Privacy_-_Part_3'
    },
    value: 42133,
    valueInMainCurrency: 42133
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: morphine.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 17421,
    valueInMainCurrency: 17421
  }
]

export const monocletePePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: monocletePe.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 32315,
    valueInMainCurrency: 32315
  },
  {
    barterItems: [
      {
        itemId: pass.id,
        quantity: 1
      }
    ],
    currencyName: 'barter',
    itemId: monocletePe.id,
    merchant: 'peacekeeper',
    merchantLevel: 3,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const ms2000Prices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'EUR',
    itemId: ms2000.id,
    merchant: 'ragman',
    merchantLevel: 1,
    quest: undefined,
    value: 95,
    valueInMainCurrency: 15105
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ms2000.id,
    merchant: 'prapor',
    merchantLevel: 1,
    quest: undefined,
    value: 15177,
    valueInMainCurrency: 15177
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: ms2000.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 26367,
    valueInMainCurrency: 26367
  }
]

export const mts25512Prices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: mts25512.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 20048,
    valueInMainCurrency: 20048
  }
]

export const mts25512DefaultPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: mts25512Default.id,
    merchant: 'jaeger',
    merchantLevel: 1,
    quest: undefined,
    value: 21316,
    valueInMainCurrency: 21316
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: mts25512Default.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 20048,
    valueInMainCurrency: 20048
  }
]

export const mts25512755mmRibPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: mts25512755mmRib.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 99000,
    valueInMainCurrency: 99000
  }
]

export const mts25512beechPrices: IPrice[] = []

export const mts25512chokePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: mts25512choke.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 2881,
    valueInMainCurrency: 2881
  }
]

export const mts25512cylPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: mts25512cyl.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 1000,
    valueInMainCurrency: 1000
  }
]

export const mts25512woodPrices: IPrice[] = []

export const nf30mmPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: nf30mm.id,
    merchant: 'jaeger',
    merchantLevel: 3,
    quest: undefined,
    value: 6313,
    valueInMainCurrency: 6313
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: nf30mm.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 178306,
    valueInMainCurrency: 178306
  }
]

export const opSksDefaultPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: opSksDefault.id,
    merchant: 'jaeger',
    merchantLevel: 1,
    quest: undefined,
    value: 36719,
    valueInMainCurrency: 36719
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: opSksDefault.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 41178,
    valueInMainCurrency: 41178
  }
]

export const opSksDtPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: opSksDt.id,
    merchant: 'jaeger',
    merchantLevel: 1,
    quest: undefined,
    value: 6318,
    valueInMainCurrency: 6318
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: opSksDt.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 15977,
    valueInMainCurrency: 15977
  }
]

export const pacaPrices: IPrice[] = [
  {
    itemId: paca.id,
    merchant: 'ragman',
    merchantLevel: 1,
    value: 29152,
    valueInMainCurrency: 29152,
    barterItems: [],
    currencyName: 'RUB',
    quest: undefined
  },
  {
    itemId: paca.id,
    value: 32043,
    valueInMainCurrency: 32043,
    barterItems: [],
    currencyName: 'RUB',
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined
  },
  {
    barterItems: [
      {
        itemId: halfMask.id,
        quantity: 3
      }
    ],
    currencyName: 'barter',
    itemId: paca.id,
    merchant: 'ragman',
    merchantLevel: 1,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const paidPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: paid.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 27326,
    valueInMainCurrency: 27326
  }
]

export const passPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: pass.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 31503,
    valueInMainCurrency: 31503
  }
]

export const plate6b33BackPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: plate6b33Back.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 43868,
    valueInMainCurrency: 43868
  }
]

export const plate6b33FrontPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: plate6b33Front.id,
    merchant: 'prapor',
    merchantLevel: 3,
    quest: undefined,
    value: 16576,
    valueInMainCurrency: 16576
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: plate6b33Front.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 28942,
    valueInMainCurrency: 28942
  }
]

export const plexiglassPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: plexiglass.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 9935,
    valueInMainCurrency: 9935
  }
]

export const precisionPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'USD',
    itemId: precision.id,
    merchant: 'peacekeeper',
    merchantLevel: 4,
    quest: undefined,
    value: 157,
    valueInMainCurrency: 22385
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: precision.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 30014,
    valueInMainCurrency: 30014
  }
]

export const pso1Prices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: pso1.id,
    merchant: 'prapor',
    merchantLevel: 1,
    quest: undefined,
    value: 19385,
    valueInMainCurrency: 19385
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: pso1.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 19562,
    valueInMainCurrency: 19562
  }
]

export const razorPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: razor.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 64132,
    valueInMainCurrency: 64132
  }
]

export const rgd5Prices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rgd5.id,
    merchant: 'prapor',
    merchantLevel: 3,
    quest: undefined,
    value: 11822,
    valueInMainCurrency: 11822
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rgd5.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 19416,
    valueInMainCurrency: 19416
  },
  {
    barterItems: [
      {
        itemId: esLamp.id,
        quantity: 1
      }
    ],
    currencyName: 'barter',
    itemId: rgd5.id,
    merchant: 'prapor',
    merchantLevel: 2,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const roosterPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rooster.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 64252,
    valueInMainCurrency: 64252
  }
]

export const rpk16Prices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 43345,
    valueInMainCurrency: 43345
  }
]

export const rpk16DefaultPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16Default.id,
    merchant: 'prapor',
    merchantLevel: 4,
    quest: undefined,
    value: 69734,
    valueInMainCurrency: 69734
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16Default.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 43345,
    valueInMainCurrency: 43345
  },
  {
    barterItems: [
      {
        itemId: capacitors.id,
        quantity: 8
      },
      {
        itemId: wParts.id,
        quantity: 2
      }
    ],
    currencyName: 'barter',
    itemId: rpk16Default.id,
    merchant: 'mechanic',
    merchantLevel: 3,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const rpk1615inchPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk1615inch.id,
    merchant: 'prapor',
    merchantLevel: 4,
    quest: undefined,
    value: 13298,
    valueInMainCurrency: 13298
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk1615inch.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 29999,
    valueInMainCurrency: 29999
  }
]

export const rpk16DrumPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16Drum.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 29263,
    valueInMainCurrency: 29263
  },
  {
    barterItems: [
      {
        itemId: alkali.id,
        quantity: 2
      }
    ],
    currencyName: 'barter',
    itemId: rpk16Drum.id,
    merchant: 'prapor',
    merchantLevel: 3,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const rpk16DustCoverPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16DustCover.id,
    merchant: 'prapor',
    merchantLevel: 3,
    quest: undefined,
    value: 7848,
    valueInMainCurrency: 7848
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16DustCover.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 53701,
    valueInMainCurrency: 53701
  }
]

export const rpk16HandguardPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16Handguard.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 50897,
    valueInMainCurrency: 50897
  }
]

export const rpk16MuzzleBreakPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16MuzzleBreak.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 12144,
    valueInMainCurrency: 12144
  }
]

export const rpk16RailPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16Rail.id,
    merchant: 'prapor',
    merchantLevel: 3,
    quest: undefined,
    value: 1154,
    valueInMainCurrency: 1154
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16Rail.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 26300,
    valueInMainCurrency: 26300
  }
]

export const rpk16RsPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16Rs.id,
    merchant: 'prapor',
    merchantLevel: 3,
    quest: undefined,
    value: 872,
    valueInMainCurrency: 872
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16Rs.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 69111,
    valueInMainCurrency: 69111
  }
]

export const rpk16RsBasePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16RsBase.id,
    merchant: 'prapor',
    merchantLevel: 3,
    quest: undefined,
    value: 872,
    valueInMainCurrency: 872
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16RsBase.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 46308,
    valueInMainCurrency: 46308
  }
]

export const rpk16TubePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16Tube.id,
    merchant: 'mechanic',
    merchantLevel: 3,
    quest: undefined,
    value: 3248,
    valueInMainCurrency: 3248
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rpk16Tube.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 21436,
    valueInMainCurrency: 21436
  }
]

export const rubPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: rub.id,
    merchant: 'prapor',
    merchantLevel: 1,
    quest: undefined,
    value: 1,
    valueInMainCurrency: 1
  }
]

export const salewaPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: salewa.id,
    merchant: 'therapist',
    merchantLevel: 2,
    quest: {
      id: '596760e186f7741e11214d58',
      name: 'Postman Pat - Part 2',
      wikiLink: 'https://escapefromtarkov.fandom.com/wiki/Postman_Pat_-_Part_2'
    },
    value: 37061,
    valueInMainCurrency: 37061
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: salewa.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 35183,
    valueInMainCurrency: 35183
  },
  {
    barterItems: [
      {
        itemId: ewr.id,
        quantity: 1
      }
    ],
    currencyName: 'barter',
    itemId: salewa.id,
    merchant: 'therapist',
    merchantLevel: 1,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  },
  {
    barterItems: [
      {
        itemId: paid.id,
        quantity: 1
      }
    ],
    currencyName: 'barter',
    itemId: salewa.id,
    merchant: 'therapist',
    merchantLevel: 1,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const scavVestPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: scavVest.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 33447,
    valueInMainCurrency: 33447
  },
  {
    barterItems: [
      {
        itemId: slickers.id,
        quantity: 1
      }
    ],
    currencyName: 'barter',
    itemId: scavVest.id,
    merchant: 'jaeger',
    merchantLevel: 1,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const slickersPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: slickers.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 16321,
    valueInMainCurrency: 16321
  },
  {
    barterItems: [
      {
        itemId: vita.id,
        quantity: 1
      }
    ],
    currencyName: 'barter',
    itemId: slickers.id,
    merchant: 'therapist',
    merchantLevel: 1,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const specterDrPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'USD',
    itemId: specterDr.id,
    merchant: 'peacekeeper',
    merchantLevel: 3,
    quest: undefined,
    value: 279,
    valueInMainCurrency: 39886
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: specterDr.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 94681,
    valueInMainCurrency: 94681
  }
]

export const srd9Prices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'USD',
    itemId: srd9.id,
    merchant: 'peacekeeper',
    merchantLevel: 2,
    quest: undefined,
    value: 242,
    valueInMainCurrency: 34606
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: srd9.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 35151,
    valueInMainCurrency: 35151
  }
]

export const syringePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: syringe.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 25320,
    valueInMainCurrency: 25320
  }
]

export const usdPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: usd.id,
    merchant: 'peacekeeper',
    merchantLevel: 1,
    quest: undefined,
    value: 143,
    valueInMainCurrency: 143
  }
]

export const vaselinePrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: vaseline.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 27714,
    valueInMainCurrency: 27714
  }
]

export const vhsPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: vhs.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 37867,
    valueInMainCurrency: 37867
  }
]

export const vitaPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: vita.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 18464,
    valueInMainCurrency: 18464
  }
]

export const waterPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: water.id,
    merchant: 'therapist',
    merchantLevel: 1,
    quest: undefined,
    value: 12401,
    valueInMainCurrency: 12401
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: water.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 17035,
    valueInMainCurrency: 17035
  },
  {
    barterItems: [
      {
        itemId: h2o2.id,
        quantity: 1
      }
    ],
    currencyName: 'barter',
    itemId: water.id,
    merchant: 'therapist',
    merchantLevel: 1,
    quest: undefined,
    value: 0,
    valueInMainCurrency: 0
  }
]

export const wPartsPrices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: wParts.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 27444,
    valueInMainCurrency: 27444
  }
]

export const x400Prices: IPrice[] = [
  {
    barterItems: [],
    currencyName: 'USD',
    itemId: x400.id,
    merchant: 'peacekeeper',
    merchantLevel: 2,
    quest: undefined,
    value: 95,
    valueInMainCurrency: 13552
  },
  {
    barterItems: [],
    currencyName: 'RUB',
    itemId: x400.id,
    merchant: 'flea-market',
    merchantLevel: 0,
    quest: undefined,
    value: 15022,
    valueInMainCurrency: 15022
  }
]



export const PriceMocks: IPrice[] = [
  ...ak12btPrices,
  ...ak12PistolGripPrices,
  ...ak12StockPrices,
  ...alkaliPrices,
  ...alphaPrices,
  ...ammo1270MagnumPrices,
  ...ammo545bpPrices,
  ...ammo545usPrices,
  ...ammo9mmGTPrices,
  ...aquamariPrices,
  ...armbandBluePrices,
  ...armor6b13FlDefaultPrices,
  ...armor6b13FlPrices,
  ...bansheeDefaultPrices,
  ...bansheePrices,
  ...bayonet6Kh5Prices,
  ...berkutPrices,
  ...capacitorsPrices,
  ...cfPrices,
  ...crossbowPrices,
  ...cultLocustPrices,
  ...ekp802dtPrices,
  ...erBayonetPrices,
  ...esLampPrices,
  ...eurPrices,
  ...ewrPrices,
  ...filterPrices,
  ...h2o2Prices,
  ...halfMaskPrices,
  ...iskraPrices,
  ...k1sPrices,
  ...k1sVisorPrices,
  ...lshZ2dtmFsPrices,
  ...lshZ2dtmPrices,
  ...m9a3CapPrices,
  ...m9a3DefaultPrices,
  ...m9a3FsPrices,
  ...m9a3MagazinePrices,
  ...m9a3Prices,
  ...m9a3RsPrices,
  ...m9a3SideGripPrices,
  ...m9a3SlidePrices,
  ...m9a3ThrPrices,
  ...mechanismPrices,
  ...monocletePePrices,
  ...morphinePrices,
  ...ms2000Prices,
  ...mts25512755mmRibPrices,
  ...mts25512beechPrices,
  ...mts25512chokePrices,
  ...mts25512cylPrices,
  ...mts25512DefaultPrices,
  ...mts25512Prices,
  ...mts25512woodPrices,
  ...nf30mmPrices,
  ...opSksDefaultPrices,
  ...opSksDtPrices,
  ...pacaPrices,
  ...paidPrices,
  ...passPrices,
  ...plate6b33BackPrices,
  ...plate6b33FrontPrices,
  ...plexiglassPrices,
  ...precisionPrices,
  ...pso1Prices,
  ...razorPrices,
  ...rgd5Prices,
  ...roosterPrices,
  ...rpk1615inchPrices,
  ...rpk16DefaultPrices,
  ...rpk16DrumPrices,
  ...rpk16DustCoverPrices,
  ...rpk16HandguardPrices,
  ...rpk16MuzzleBreakPrices,
  ...rpk16Prices,
  ...rpk16RailPrices,
  ...rpk16RsBasePrices,
  ...rpk16RsPrices,
  ...rpk16TubePrices,
  ...rubPrices,
  ...salewaPrices,
  ...scavVestPrices,
  ...slickersPrices,
  ...specterDrPrices,
  ...srd9Prices,
  ...syringePrices,
  ...usdPrices,
  ...vaselinePrices,
  ...vhsPrices,
  ...vitaPrices,
  ...waterPrices,
  ...wPartsPrices,
  ...x400Prices
]