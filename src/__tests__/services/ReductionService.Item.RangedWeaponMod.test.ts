...
describe('parseReducedItem', () => {
  it.each([
    ...
    // Act
    const rangedWeaponModResult = service.parseReducedItem(reducedRangedWeaponMod)

    // Assert
    expect(rangedWeaponModResult.success).toBe(true)
    expect(rangedWeaponModResult.value).toStrictEqual(expected)
  })
})