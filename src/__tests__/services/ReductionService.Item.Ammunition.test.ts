...
describe('parseReducedItem', () => {
  it.each([
    ...
    // Act
    const ammunitionResult = service.parseReducedItem(reducedAmmunition)

    // Assert
    expect(ammunitionResult.success).toBe(true)
    expect(ammunitionResult.value).toStrictEqual(expected)
  })
})