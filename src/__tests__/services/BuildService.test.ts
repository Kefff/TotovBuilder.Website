...
import { default as WebsiteConfigurationMock, default as websiteConfigurationMock } from '../__data__/websiteConfigurationMock'
...
describe('BuildService', () => {
  ...
  describe('toSharableUrlAsync', () => {
    it.each([
      [
        build1,
        'localhost:3000/s/XQAAAALCBAAAAAAAAABBqEnKciJ9Ha4afmksn3Ip09U9oadkP4_cw4Odt9duEGxP6hDGHXtCf4YXExsq19d1fc_JfV2HlE2Qn2ymiCpdwo4P0GtIxFO3zQJKMzoZgu3N1eJ0uns_4cplHCPi-leRdtMS2DkY39yfqkZjIeBxG2M42EQZG3rMvjCxY0FoMscPiKU-TOWM5zVI9Lev5FYS_ExdiGKhfRPKYujX_9FjD8H9SmI8Df57uekfqpY0z7Vz5B4A-AY-C4vInLbJwEYD_EqKBBCwCGCHm0DDpj0goOP7_Iudalkmk1n6hA8gUq9VAQ31brHMQtGYeFsXLakpDf1rG4x9GSHaaZS28N85Hz9I3e8NJdUB1h35M8x2XsvzpUPBDfar5IN3HO3_xwuY2_yZ0YYrUNKKpfiFv1XHlCJsf2mYp2-hc_wCsWQWZsHvb1TXbEzeC0pLSW-FqOq33l9lNLfOVy2T2iqsOYIoHhXsqz696C51tHw-mXeR66o18px-fP4ZNhBsv6N_JG53Wo_2U0VwhJgjeFM5plkEvQ-10zViI0ylHKOxEA4NGuvqNKdPzO_bBaUGXUKx84LcSjzkmCCWxyBTSO4fmn8xOBa4vBn14952JkUpI7YXAo6_sKURuap4Dqy4FWJGeJFGIVXalwm-IyZxkH_gjoqvJ4DMO-j31vJT1RTYZ-bYsHtnLyaU-H6uTVb-ooitO3Z6pmZQyBYidNjPEXMUlE_nk6ChnPWMC-cjnLjPep553h4tpbXPhPSx_L-vTXzDod10x87geMvXldfDtHlVf_YzQRwQqCiMQWfCWE8LwSxMqr-4VLEGOlPtGxavKmtqe_6qiaI'
      ],
      [
        build2,
        'localhost:3000/s/XQAAAAI_AwAAAAAAAABBKEnKciJ9Ha4afmlhjXIcBHJ5OAjWBvHRqhzsw2sFohvtE2U5Ax-ZhpnJP5jm2hvuJmbR_88c5MLjq2AZyyIReyJ-7BxYduIOn4n0fu2tfBOvPNWlcixwLZO1VGePLUD5o2Ecs8J4dbz6zB1DvdfOl7I1zHA3gjt9_78XznrP3_PAQg3DejFaHp3dULJQyxzqwNiDs3OOUfIwRGFd5S-urvsBPs1_gEtIudOzGEfBBy20xD6GrV-QjaQKiRUfU4yV1ws9tuIeuyZzbg2QP1cON2MQ8vR5D6eHm2-MWlJjwHIwf4EnifB7mO4WnufIc_i8KD9ExoEPEtbTQpEa-2hVWnVCN_Oo7fL7HxVOvER-x5ExV57LX-gjvmbJ2Fnu_NruEzqyI8kktrxs0RfNo3ZRjArb-0TGqLRhTXsA4q3PuT5_zGsl4dNmidYYtG_dKZDAEXpv0OXo_ACd3rHXVSb7164luG7uqSFJX9gUKvM7w65AqVMEnHltm-pmNer4wgY-3Ie8EDEab8kMzo-hY7JD4zK2s55lopW4QgGNOXdi6ElV07V5v-3Ju9j_IjHjp3KX9WFqDdQAys8nXJho-iIuSwFnbGbawYaht_op0H-SCv9Bj38A'
      ]
    ])('should reduce a build, transform it into a URL and save the URL in the build', async (build: IBuild, expectedLongSharableString: string) => {
      // Arrange
      useVersionServiceMock()
      useWebsiteConfigurationServiceMock()

      build = { ...build } // Creating a new build to avoid the one being passed to be changed for all tests

      Services.configure(ReductionService)

      const expectedSharableUrl = `${websiteConfigurationMock.endpointUrlShortener}/wNCBVt`

      const urlShortenerServiceMock = mock<UrlShortenerService>()
      when(urlShortenerServiceMock.shortenAsync(expectedLongSharableString)).thenReturn(Promise.resolve(expectedSharableUrl))
      Services.configure(UrlShortenerService, undefined, instance(urlShortenerServiceMock))
      ...
      // Assert
      expect(sharableUrl).toBe(expectedSharableUrl)
      expect(build.sharabledUrl).toBe(expectedSharableUrl)
      verify(serviceSpy.updateAsync(build)).once()
    })

    it('should return the build sharable URL if it has one', async () => {
      // Arrange
      const existingSharableUrl = `${WebsiteConfigurationMock.endpointUrlShortener}/test`

      const build: IBuild = {
        ...build1,
        sharabledUrl: existingSharableUrl
      }
      ...
      // Assert
      expect(sharableUrl).toBe(existingSharableUrl)
      verify(serviceSpy.updateAsync(build)).never()
    })
